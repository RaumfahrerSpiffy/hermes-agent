        ...
        for endpoint in token_endpoints:
            req = urllib.request.Request(
                ...
                headers={
                    ...
                    # Upstream #48534: the token endpoint must NOT see a claude-code/ UA
                    # (triggers 429s on login/refresh). API requests keep the
                    # claude-code fingerprint; only the OAuth token calls use this.
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    # MERCURY FORK (2026-07-06): refresh_expired=False — this env var is
    # the designated long-lived setup-token slot, so an expired credential
    # file must never gate it behind a doomed network refresh. A currently-
    # valid file credential still wins (fresh CLI login beats old setup token).
    cc_token = _getenv("CLAUDE_CODE_OAUTH_TOKEN").strip()
    if cc_token:
        preferred = _prefer_refreshable_claude_code_token(
            cc_token, _read_creds(), refresh_expired=False
        )
        if preferred:
            return preferred
        return cc_token
