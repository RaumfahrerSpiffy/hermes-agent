...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    out = result.stdout
    # MERCURY FORK (f7f7fa1c9): brand name is fork-configurable — read it from
    # build_info rather than asserting the literal.
    from hermes_cli.build_info import get_brand_name
    for field in (f"{get_brand_name()} v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    # MERCURY FORK (f7f7fa1c9): brand from build_info, not the literal.
    from hermes_cli.build_info import get_brand_name
    assert f"{get_brand_name()} v" in result.stdout
