...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Mercury')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#F5F7FA')
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    // MERCURY FORK: the compiled-in DARK_SEEDS carry the Mercury silver-steel
    // identity, so the derivation contract is tested against upstream's
    // original gold seeds fed to buildPalette() inline — the MATH contract
    // survives while the compiled-in identity diverges.
    const goldDark = dark.buildPalette(
      {
        accent: '#FFBF00',
        activeRow: '#333355',
        bg: '#101014',
